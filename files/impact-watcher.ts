...
function ensureBeadsInitialized(worktree: string): boolean {
  ...
  const result = runCommand(BD_PATH, ["init", "--quiet", "--stealth", "--prefix", sanitized], worktree, 10000)
  return result !== null
}

function parseBeadsReady(worktree: string): BeadsTask[] {
  const json = runCommand(BD_PATH, ["ready", "--json"], worktree, 8000)
  if (!json) return []
...
function parseBeadsOpen(worktree: string): BeadsTask[] {
  const json = runCommand(BD_PATH, ["list", "--status", "open", "--json"], worktree, 8000)
  if (!json) return []
...
function runBeadsSync(worktree: string): void {
  ...
  runCommand(BD_PATH, ["dolt", "commit"], worktree, 10000)
  runCommand(BD_PATH, ["dolt", "push"], worktree, 10000)
}
